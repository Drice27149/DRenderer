...
float SurfelGenerator::RandNorm()
{
    return (float)rand() / (float)RAND_MAX;
}

vec3 SurfelGenerator::RandBarycentricCoordinates()
{
    float a = RandNorm(), b = RandNorm();
    return vec3(1.0-sqrt(a), sqrt(a)*(1.0-b), sqrt(a)*b);
}

std::vector<Surfel> SurfelGenerator::GenerateSurfels(std::vector<WorldTriangle>& trians, int surfelCnt)
{
    srand(time(NULL));
...
std::vector<Surfel> SurfelGenerator::MergeSurfels(std::vector<Surfel>& surfels)
{
    return surfels;
}

std::vector<SurfelCluster> SurfelGenerator::GenerateSurfelClusters(std::vector<Surfel>& surfels, int maxSurfelPerCluster)
{
